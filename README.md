...
- **抓取端**：GitHub Actions 每天 00:37（America/Los_Angeles）运行 `scripts/fetch-sources.js`，按 `config.yaml` 里声明的源逐个抓取，过滤到刚结束的美国太平洋日，写入 `data/fetch-cache/YYYY-MM-DD.json`；03:17 的兜底任务只补抓缺失或失败来源，完整缓存不会产生重复网络请求。
...
- **[socialdata.tools](https://socialdata.tools)** — 付费 Twitter REST API。Twitter 源优先按目标日期使用带 `since_time` / `until_time` 的 Search 接口并完整翻页；搜索为空或首屏失败时，用稳定 user ID 的时间线完整校验。选它的原因：自建 RSSHub + 新小号 cookie 会被 Twitter 截断 timeline（新账号只返回 1-17 条而不是完整 20 条），socialdata 自己维护账号池拿完整结果。当前按返回的 tweet 计费，约 $0.0002/条。
...
- **`api.socialdata.tools`** — 付费 Twitter REST API，所有 `*(Twitter)` 源走它。需一个 GH Actions secret：`SOCIALDATA_API_KEY`（dashboard 里生成）。路由文件硬编码 `handle` 和 `userId`（Twitter 数字 ID 不会随 handle 改名变化）。按返回的 tweet 数量计费；每个来源的原始返回量记录在 fetch-cache 的 `api_usage.returned_tweets`。**失效信号**：全部 Twitter 源集体 `error`（API key 被吊销、余额耗尽或服务宕机），3 天连续失败后开 `source-alert` issue。
