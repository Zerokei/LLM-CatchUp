...
- **抓取端**：GitHub Actions 每天 01:37（America/Los_Angeles）跑一次 `scripts/fetch-sources.js`，按 `config.yaml` 里声明的源逐个抓取，过滤到刚结束的美国太平洋日，写入 `data/fetch-cache/YYYY-MM-DD.json`。
